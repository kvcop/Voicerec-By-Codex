...
3. **Подключить PostgreSQL**
   - Использовать `AsyncSessionLocal` и `get_session` из `backend/app/db/session.py`, без создания новых фабрик
   - Настроить переменную окружения `DATABASE_URL` для локального и продакшн-окружений
