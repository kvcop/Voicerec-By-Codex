...
2. **Перенос TranscriptService в модуль сервисов**
   - Вынести `TranscriptService` из `app/api/meeting.py` в `app/services/transcript_service.py`
   - Настроить DI через `Depends` и конструктор, внедряя репозитории и gRPC-клиенты
   - Обеспечить покрытие сервиса unit-тестами

3. **Реализация TranscriptService**
   - Асинхронная отправка аудио на транскрипцию
   - Получение результатов и сохранение в БД
   - Стриминг результатов через SSE
   - Обновление существующих тестов стриминга под новую структуру зависимостей

4. **Интеграция с GPU сервисами**
